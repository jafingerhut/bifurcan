 ...
 * @author ztellman
 */
public class ListNodes4 {

  // Temporarily use a smaller shift value, so generative tests can
  // find problems much more quickly.  Smaller size List4 objects
  ...
  public static class Node {
    ...
    public Node set(Object editor, int idx, Object value) {
      ...
      if (shift == SHIFT_INCREMENT) {
        nodes[nodeIdx] = ListNodes4.set((Object[]) nodes[nodeIdx], idx - offset(nodeIdx), value);
      } else {
    ...
    public Node concat(Node node, Object editor) {
      ...
      if (shift == node.shift) {
        ...
        for (int i = 0; i < node.numNodes; i++) {
          newNode = ListNodes4.pushLast(newNode, node.nodes[i], editor);
        }
    ...
    public Node slice(int start, int end, Object editor) {
      ...
      if (startIdx == endIdx) {
        ...
        Object child = ListNodes4.slice(nodes[startIdx], editor, start - offset, end - offset);
        if (shift > SHIFT_INCREMENT) {
          return (Node) child;
        } else {
          rn = ListNodes4.pushLast(rn, child, editor);
        }
        ...
        rn = ListNodes4.pushLast(rn, ListNodes4.slice(nodes[startIdx], editor, start - sLower, sUpper - sLower), editor);

        // intermediate full nodes
        for (int i = startIdx + 1; i < endIdx; i++) {
          rn = ListNodes4.pushLast(rn, nodes[i], editor);
        }

        // last partial node
        int eLower = offset(endIdx);
        rn = ListNodes4.pushLast(rn, ListNodes4.slice(nodes[endIdx], editor, 0, end - eLower), editor);
      }
